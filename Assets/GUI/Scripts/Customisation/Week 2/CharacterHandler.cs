...
public class CharacterHandler : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUI.Box(new Rect(6 * scrW, .25f * scrH, 4 * scrW, .5f * scrH), ""); //background
        ...
        GUI.Box(new Rect(6 * scrW, .25f * scrH, curHealth * (4 * scrW)/ maxHealth, .5f * scrH), ""); //moving health bar
        //GUI Box on screen for the experience background
        GUI.Box(new Rect(6 * scrW, .75f * scrH, 4 * scrW, .25f * scrH), ""); //background
        ...
        GUI.Box(new Rect(6 * scrW, .75f * scrH, curExp * (4 * scrW)/ maxExp, .25f * scrH), ""); //moving exp bar
        //GUI Draw Texture on the screen that has the mini map render texture attached
        GUI.DrawTexture(new Rect(13 * scrW, .25f * scrH, 2.925f * scrW, 2.5f * scrH), miniMap); //minimap
    }
    #endregion
}