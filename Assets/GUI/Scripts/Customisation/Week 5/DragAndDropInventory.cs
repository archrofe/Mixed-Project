...
public class DragAndDropInventory : MonoBehaviour
{
    ...
    void DrawItem(int windowID)
    {
        ...
        {
            GUI.DrawTexture(new Rect(0, 0, scrW * .5f, scrH * .5f), draggedItem.Icon);
        }
    }
    #endregion
    #region ToolTip
    ...
    {
        GUI.Box(new Rect(0, .25f * scrH, 6 * scrW, .5f * scrH), "Banner");
        GUI.Box(new Rect(0, 4.25f * scrH, 6 * scrW, .5f * scrH), "Gold & Exp");
        showToolTip = false;
        ...
        for (int y = 0; y < slotY; y++)
        {
            ...
            {
                Rect slotLocation = new Rect(scrW * .125f + x * (scrW * .75f), scrH * .75f + y * (scrH * .65f), .75f * scrW, .65f * scrH);
        ...
        #endregion
        #region Drag Window
        GUI.DragWindow(new Rect(0 * scrW, 0 * scrH, 6 * scrW, .5f * scrH));//Top Drag
        GUI.DragWindow(new Rect(0 * scrW, .5f * scrH, .25f * scrW, 3.5f * scrH));//Left Drag
        GUI.DragWindow(new Rect(5.5f * scrW, .5f * scrH, .25f * scrW, 3.5f * scrH));//Right Drag
        GUI.DragWindow(new Rect(0 * scrW, 4 * scrH, .25f * scrW, 3.5f * scrH));//Bottom Drag
        #endregion
    }
    #endregion
    #region Start
    ...
    void OnGUI()
    {
        ...
        {
            toolTipRect = new Rect(e.mousePosition.x + .01f, e.mousePosition.y + .01f, scrW * 2, scrH * 3);
        ...
        if (dragging)
        {
            if (draggedItem != null)
            {
                Rect mouseLocation = new Rect(e.mousePosition.x + .125f, e.mousePosition.y + .125f, scrW * .5f, scrH * .5f);
